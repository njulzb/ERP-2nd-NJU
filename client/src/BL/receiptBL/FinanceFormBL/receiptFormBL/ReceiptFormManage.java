package BL.receiptBL.FinanceFormBL.receiptFormBL;

import java.rmi.RemoteException;
import java.util.ArrayList;

import DataService.receiptDataService.FormDataService;
import RMI.ClientRunner;
import po.KeyForSearchForm;
import po.formPO.FormPO;
import po.formPO.financeFormPO.ReceiptFormPO;
import vo.Form.FormVO;
import vo.Form.FinanceFormVO.ReceiptFormVO;

/**
 * 真正对form进行管理的类
 * @author lzb
 *
 */
public class ReceiptFormManage {
	private FormDataService rmi;

	public ReceiptFormManage() {
		rmi = ClientRunner.getInstance().getFormDataService();
	}

	public FormVO getForm(String id) {

		FormPO c = null;
		try {
			c = rmi.getForm(id);
		} catch (RemoteException e) {
			// TODO 自动生成的 catch 块
			System.out.println("未成功getFormPO  #83883");
			e.printStackTrace();
		}
		ReceiptForm temp = new ReceiptForm((ReceiptFormPO) c);

		return temp.toVO();
	}

	public boolean addForm(FormVO toPush) {
		ReceiptForm temp = new ReceiptForm((ReceiptFormVO) toPush);
		boolean flag = false;
		try {
			flag = rmi.newForm(temp.toPO());

		} catch (RemoteException e) {
			// TODO 自动生成的 catch 块
			System.out.println("未成功newFormPO  #8343");
			e.printStackTrace();
		}

		return flag;
	}

	public boolean deleteForm(String id) {
		boolean flag = false;
		try {
			flag = rmi.deleteForm(id);
		} catch (RemoteException e) {
			// TODO 自动生成的 catch 块
			System.out.println("未成功newFormPO  #12883");
			e.printStackTrace();
		}
		return flag;
	}

	public boolean editForm(FormVO toPush) {
		boolean flag = false;
		ReceiptForm temp = new ReceiptForm((ReceiptFormVO) toPush);
		try {
			flag = rmi.editForm(temp.toPO());
		} catch (RemoteException e) {
			// TODO 自动生成的 catch 块
			System.out.println("未成功newFormPO  #12883");
			e.printStackTrace();
		}
		return flag;
	}

	/**
	 * 根据关键字搜索单据
	 * @param key
	 * @return
	 */
	public ArrayList<FormVO> searchForm(KeyForSearchForm key) {
		ArrayList<FormVO> out = new ArrayList<FormVO>();
		ArrayList<FormPO> temp = new ArrayList<FormPO>();
		try {
			temp = rmi.searchForm(key);
		} catch (RemoteException e) {
			System.out.println("未成功searchFormPO  #83343");
			// TODO 自动生成的 catch 块
			e.printStackTrace();
		}
		for (FormPO a : temp) {
			out.add(new ReceiptForm((ReceiptFormPO) a).toVO());
		}

		return out;
	}

	/**
	 * 审批通过单据
	 * @param id
	 * @return
	 */
	public boolean passForm(String id) {
		ReceiptForm temp = new ReceiptForm((ReceiptFormVO) getForm(id));
		temp.pass();
		editForm(temp.toVO());
		return true;

	}
}
